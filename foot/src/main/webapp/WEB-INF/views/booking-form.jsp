                        ...
                        <div class="col-md-8">
                            ...
                            <p class="mb-2">
                                ...
                                <span class="text-success fw-bold">
                                    <fmt:formatNumber value="${stadium.pricePerHour}" pattern="#,###"/> VNĐ/giờ
                                </span>
                            </p>
                            <p class="mb-0">
                    ...
                    <form:form action="${pageContext.request.contextPath}/stadiums/${stadium.id}/book"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-select" id="startTime" name="startTime" required disabled>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const oldStartTime = "${oldStartTime}";
    const oldEndTime = "${oldEndTime}";
    ...
    async function fetchBookedTimeSlots(date) {
        try {
            const response = await fetch(`${contextPath}/api/bookings/check-availability?stadiumId=${stadiumId}&date=${date}`);
    ...
    function updateStartTimeOptions() {
        ...
        if (oldStartTime) {
            const oldOption = Array.from(startTimeSelect.options).find(opt => opt.value === oldStartTime && !opt.disabled);
            if (oldOption) {
    ...
    function updateEndTimeOptions() {
        ...
        if (oldEndTime) {
            const oldOption = Array.from(endTimeSelect.options).find(opt => opt.value === oldEndTime && !opt.disabled);
            if (oldOption) {
    ...
    startTimeSelect.addEventListener('change', function() {
        // Khi user tự đổi startTime thì không giữ oldEndTime nữa
        updateEndTimeOptions();
    });
